...
thinkingStrategy 可选值：
- "auto"（默认、顺从模型 DB 默认配置）
- "enabled"（强制启用推理/思维模式）
- "disabled"（强制关闭推理/思维模式）
不确定时省略该字段或填 "auto"。
...
- "hasModality": 多模态内容类型匹配，需要 modalities 字段（字符串数组，可选值: "vision", "audio", "image-generation"）
...
thinkingStrategy (optional):
- "auto" (default — defer to model DB configuration)
- "enabled" (force thinking/reasoning mode on)
- "disabled" (force thinking/reasoning mode off)
Omit or use "auto" when unsure.
...
- "hasModality" → modalities: ["vision"] and/or ["audio"] and/or ["image-generation"]
...
可选字段 thinkingStrategy（"auto" | "enabled" | "disabled"）：控制该规则匹配时是否启用/关闭思维推理模式。默认 "auto"（沿用模型配置）。
...
Optional field thinkingStrategy ("auto" | "enabled" | "disabled"): controls thinking/reasoning for matched requests. Default "auto" (defer to model DB config).
...
- thinkingStrategy: auto | enabled | disabled（可选，控制思维推理模式）
...
- thinkingStrategy: auto | enabled | disabled (optional, controls thinking/reasoning mode)
